...
- **character — 生命週期 token(R8 #6 → R9 #3)**:`Character.lifecycleRevisions` 是**逐種類**的 token(`adulthood`/`retirementCheck`/`naturalDeathCheck`),`characterLifecycleDue` 比對該種類自己的值。兩個都不要退回去:① 改用 `character.revision` → 受傷/狀態變更會把所有 Job 誤判成過期(R8 #6);② 併回單一 token → 退休會連帶作廢自然死亡 Job,退休角色永遠不會老死(R9 #3)。三者的失效條件本來就不同。新增角色的建構點都必須帶這個欄位(型別已強制)。
