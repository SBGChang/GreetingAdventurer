...
- **內容 Reader adapter 骨架**:`src/app/content/reader-adapter.ts`(通用 `domainDefinitionView` / `narrowedDomainReader`)+ `dungeon-reader.ts`(`createDungeonDefinitionReader`,data-runtime 的**第一個真實消費者**,其餘模組 reader 的樣板)。單元 + 端到端皆證(真 reader 換進 Session 跑 openDungeonDoor 供給 redDoorOpenMinutes)。**待做**:其餘模組 reader(map/inventory/combat/team/progression/character)依此樣板擴充;resolver adapter 半邊(registry→`ResolverRegistration`.resolve→§7.1 kernel+params→模組 ResolverPort)尚未建。
