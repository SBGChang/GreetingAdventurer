...
import {
  ...
  firstMapLayouts,
  weaponDpsReview,
} from './docs/03_content/yunhua/yunhua_content.data.mjs';

const output = new URL('./docs/03_content/yunhua/yunhua_content.html', import.meta.url);
...
<html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>雲華｜內容與平衡審閱</title>
<style>
...
</style></head><body><main><h1>雲華｜內容與平衡審閱</h1><p class="subtitle">2026-08-06・第一版可玩內容的數值基準。這是內容平衡閱讀頁，不是 Runtime Definition JSON；正式資料仍應依資料契約拆檔。</p>${filterStyles}<nav class="filters" aria-label="內容篩選">${navigation}</nav>${renderBalance()}${renderEquipment()}${renderSkills()}${renderMonsters()}${renderItems()}${renderCrafting()}${renderMaps()}</main>${filterScript}</body></html>`;
