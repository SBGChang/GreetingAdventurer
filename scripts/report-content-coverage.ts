...
const registered = new Set(loaded.resolverBindings.map(binding => String(binding.resolverId)));
const missingResolvers = [...resolverIds].filter(id => !registered.has(id)).sort();
console.log(`已提供 binding：${registered.size}；內容引用但沒有 binding：${missingResolvers.length}`);
console.log('正式 shape 實作由 verify:ui 的開局載入驗證；binding 不等於所有領域 port 已接線。');
for (const id of missingResolvers) console.log(`   ✗ ${id}`);
...
console.log('');
console.log('這是有資料的種類比例，不是遊戲完成度；有 Schema 或資料不代表 Handler、正式 port 與完整流程已可用。');
console.log('實作範圍見 docs/CURRENT_STATUS.md，正式路徑以 verify:ui 驗證。');
