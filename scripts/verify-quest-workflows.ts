...
try {
 ...
 {const old=gunzipSync(readFileSync('src/testing/fixtures/pre-guild-upgrade.save.json.gz')).toString('utf8');const oldState=JSON.parse(JSON.parse(old).payload).state;game=createGame(config,old);v=game.view;const upgraded=state();assert.equal(upgraded.core.worldDay,oldState.core.worldDay);assert.deepEqual(upgraded.character,oldState.character);assert.deepEqual(upgraded.team,oldState.team);assert.deepEqual(upgraded.economy,oldState.economy);for(const q of Object.values(oldState.quest.quests) as any[])assert.deepEqual(upgraded.quest.quests[q.questId],q);assert.equal(Object.values(upgraded.map.contents).filter((c:any)=>c.kind==='monsterGroup').length,33);assert(Object.values(upgraded.quest.quests).some((q:any)=>q.kind==='delivery'));resume();console.log('SAVE UPGRADE PASSED: day, team, characters, money and previous quests preserved; missing encounters and cargo quests added');}
