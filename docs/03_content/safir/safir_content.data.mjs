import { createCultureData, ability, damage, counter, heal, support, stats, monsterSkill, floor } from '../shared/content_factory.mjs';
...
const mapLayouts = [
  {name:'棕影鹽澤',city:'赤帆驛',type:'一般野外型；8×8 單層',floors:[floor('單層鹽澤：棕影路脊',['##E..###','#....R##','...##...','.?....B.','..!..##.','...R...#','#..T....','###...X#'],'白鹽地、棕櫚影與乾涸水道交錯，風標與迷目是主要壓力。')]},
  {name:'埋砂商站',city:'星井城',type:'一般建築型；5×5 地上／地下',floors:[floor('地上 1F：傾斜貨廳',['#E.##','...R#','.!S..','#..T.','##..X'],'地上層保留商隊貨架、紅門庫房與坍塌入口。'),floor('地下 1F：埋貨獸穴',['##.##','#...#','.RS..','.?..#','#B###'],'地下層以沙穴、舊貨囊與埋商吞獸為核心。')]},
  {name:'月環星臺',city:'星井城',type:'建築型國家迷宮；6×6 塔 1～4F＋地下 1～2F',floors:[floor('塔 1F：迎星門廳',['##E###','#....#','..!..#','.R.S.#','#..T.#','######'],'入口層建立路誓石衛與星盤機關。'),floor('塔 2F：折光迴廊',['######','#T...#','#..!.#','#..S.#','#?...#','######'],'折光紅門改變探索路線，但不揭露未知格。'),floor('塔 3F：月塵書庫',['######','#.R..#','#....#','#..S.#','#B...#','######'],'月塵蛾與失星回聲共同施加迷目、失途。'),floor('塔 4F：九曜盤室',['######','#...T#','#.S..#','#..?##','#....#','######'],'盤室集中高階星砂、書籍與菁英遭遇。'),floor('地下 1F：失路基座',['######','#B...#','#..!.#','#..S.#','#.R..#','######'],'舊觀測基座與私掘隊占據地下通道。'),floor('地下 2F：月環心室',['######','#....#','#.B..#','#....#','#T..X#','######'],'最深層為月環星獸 Boss 與星核寶箱。')]},
];
const mapConfigs = [
