...
| `teamPlanDue` | team | 旅行段落、到達城市、進出冒險地等大動作。 |
...
|---|---|
| `ItemDefinition` | `ItemInstance`：實體 ID、擁有者、耐久／隨機詞綴等。 |
...
| `ContentEventDefinition` | `ContentEventInstance`：位置、條件、是否已處理。 |

`ContentEvent` 不另設一個可任意寫全世界的 Event 模組。它是資料定義與已選定的內容快照，Runtime Instance 由發生情境的領域保存：

- 旅行事件由 Team 的旅行 Plan／Pending Interaction 保存。
...
若途中產生必須由玩家選擇的旅行／世界事件，擁有模組先把 Pending Interaction 寫入 State，世界推進在該日提交並停止。UI 解決互動後才繼續原本的目標日；不可把選擇只存在 React Modal 或非存檔 Promise。
...
| `team` | 玩家／NPC 隊伍、位置、大動作、成員自由行動與近期行動紀錄；不擁有共用金錢或一般物品。 | 行動規則、旅行規則、近期紀錄規則。 | `TeamPlanChanged`、`TeamLocationChanged`、`TeamMemberJoined`。 |
| `adventurer-lifecycle` | NPC Team 的 Controller、下一個意圖、動作串與市場交易意圖；不執行或擁有隊伍／任務／資產。 | 決策、動作串、個人自由行動與市場偏好規則。 | `NpcIntentSelected`、`NpcActionChainChanged`、`NpcMarketIntentCreated`。 |
| `dungeon` | 玩家探索暫態、`NpcDungeonRun`、NPC 骰定暫存結果。 | NPC 探索點規則、內容 Resolver。 | `NpcDungeonRunProgressed`；以 Internal Command 要求 Map 結算。 |
| `city` | 建築狀態、商店刷新政策、貨架 Offer、護衛候選，以及玩家主角每日聊天／交易上限。 | 城市與設施定義、商店規則、玩家主角每日交流限制。 | `ShopRefreshed`、`EscortCandidatesGenerated`、玩家交流完成事件。 |
| `inventory` | 所有 Item Instance 的所有權、唯一位置、保留與移轉紀錄。 | 道具定義。 | `InventoryTransferred`、`ItemConsumed`。 |
| `quest` | Quest Instance、兩期限、四狀態、接取與結案。 | 委託規則、反應規則。 | `QuestCreated`、`QuestStateChanged`、`QuestSettled`。 |
| `progression` | 熟練度等級／MXP、主屬、傳授進度。 | 成長曲線、MXP 表、書籍門檻。 | `MasteryExperienceGranted`、`MasteryLevelChanged`。 |
| `combat` | 戰鬥遭遇、雙方九宮格 Runtime、倒扣式 CTB、同值排序、前排全空補位、技能與戰鬥結果。 | 怪物、技能、開場 CTB、行動延遲與裝備係數。 | `CombatEncounterResolved`、`CombatAttackMasteryEarned`。 |
...
兩者都是 Gameplay Formula，不是 ViewModel，也不得建立自己的 GameState Slice。
...
NPC 地牢探險採抽象日結算，不走玩家的格子、紅門與陷阱邏輯。地圖生成時，`map` 模組對動態內容建立有序序列；玩家讀取 `position` 探索，NPC 讀取 `npcOrder`。
...
type NpcDungeonRun = {
  ...
  status: 'exploring' | 'leaving' | 'settling' | 'closed';
...
3. 點數足夠才對下一筆目標執行一次資料指定的 NPC Resolver；結果寫入 `pendingResults`。採集成功時把完整 deterministic Resolution 暫存，Settlement 日不得重骰。
4. 點數不足時，保留游標與暫存結果，排入明日 `npcDungeonDay`。
5. 全序列完成、任務目標完成、首次戰敗或 Resolver 要求離場時，進入 `settling`。
6. 結算時才由 `map` 正式套用仍有效的內容結果；後續由 `inventory`、`progression`、`quest` 分別處理，正式戰利品則加入該 Run 的 Asset Distribution。
7. NPC Distribution 逐件 RNG 指派物品、平均分配貨幣；完成後 Dungeon 才關閉 Run 並讓 Team 返城。
...
| 酒館聊天、近期行動、招募、解雇與隊伍異動；只可招募單人 Team、多人隊伍一律拒絕挖角 | Team + City Read Model。 |
