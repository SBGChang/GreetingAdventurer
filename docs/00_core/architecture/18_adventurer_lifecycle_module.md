...
>
> **依賴：** [共用核心契約](00_shared_contracts.md)、Team／Quest／City／World／Map／Dungeon／Inventory／Economy／Character 的公開 Query，以及 Combat 的 NPC 戰力估算 Port。
>
> **責任：** 管理非玩家冒險者（包含單人與多人隊伍）的下一個意圖、動作串、任務承諾與城市生活選擇。它決定「NPC 接下來想做什麼」；不執行旅行、地牢、交易、製作、訓練或委託本身。
...
type NpcMemberFreeActionCandidateRule = {
  actionKind: 'craft' | 'train' | 'trade' | 'rest';
...
type ActionChainNodeTemplate =
  | { kind: 'cityFree' }
  | { kind: 'travelToCity'; destinationResolverId: ResolverId; travelModeResolverId: ResolverId }
  ...
  | { kind: 'travelToQuestTarget'; destinationResolverId: ResolverId; travelModeResolverId: ResolverId }
  | { kind: 'enterQuestMap' }
...
| `FreeActionCompleted` | 對 craft／train 走既有完成流程；對 `trade` 建立一筆資料化 `NpcMarketIntent`，交由市場 Workflow 執行。 |
...
|---|---|---|
| `StartNpcTeamPlan` | team | 啟動旅行、進圖、返城或城市自由活動的實際 Plan。 |
| `AssignNpcMemberFreeAction` | team | 在 `cityFree` 中為個別正式成員建立 craft／train／trade／rest。 |
...
| `StartNpcDungeonRun` | dungeon | 開始目標對應的抽象地牢 Run。 |
...
`enterNearbyAdventureMap`（對應模板的 `executeNearbyAdventure`）是一節邏輯節點：必要時先至附近目標城，再進入對應冒險地、啟動 NPC Dungeon Run、離開地圖，最後完成該節。Run 首次失敗或有序序列完成後便結束，接著強制進入自由活動。`travelToCity` 也只包含一段合法城市旅行；抵達後同樣先進入強制自由活動，不能直接重抽。
...
acceptNearbyQuest
  ...
  → Combat Power Estimator 排除不可行項並依預期成功權重抽選
...
```text
cityFree
  → 正式成員各自抽 craft / train / trade / rest
