...
`.html`、`content_factory.mjs`、兩支 builder，並新增 `adventure_structure.mjs`）。因此這輪**只動**
`src/`、`shared/map_model.mjs`、`shared/map_render.mjs`，不碰內容資料、不重新產生 HTML。

**#1 equipItem 仍漏掉被清空武器組的事件** —— R12 只補了 `configureWeaponSet`。而且 equipItem 還有第二個
錯：對**每一件**被頂掉的物品都套用 `cmd.slotId`，於是被替換的雙手武器（占主手）會被回報成「命令的那個
slot」清空了。**根因是手寫發送點**——R11 補本組、R12 補其他組、R13 補另一條入口，一次補一個呼叫點永遠追
不完。改：抽出 `equipmentChangeEvents`，**比對整份 Loadout 前後差異**，位置變了就一定發、slot 一定取自該
位置本身；兩條入口共用，不會再分岔。

**#2 已到期的刷新鎖仍擋住新委託** —— 刷新流程（§371）與 `isRefreshLocked` 都以 `releaseOnDay <= worldDay`
判定失效，但 R12 我加的 set 防線只看鎖**存不存在**。舊委託的殘留鎖因此永遠擋住新委託。改：只有仍生效的
鎖才算被持有。**同一個概念在三處各判一次，其中一處判法不同**——這是 #1 的另一種形狀。

**#3 所有上樓梯都被畫成下樓梯** —— 渲染器比對 `room.stair === 'up'`，但資料與驗證器用的是箭號 `↑`／`↓`，
於是**每一座**樓梯都落到 else 分支畫成 ↓（四國全圖 0 個 ↑）。這段判斷是我在 R12 從雲華舊 renderer **原樣
抄過來**的——抽共用時我驗了「抽出來的行為和原本一致」，卻沒驗「原本是對的」。忠實地複製了一個一直存在的
bug。

**#4 合法的多出口地圖被拒** —— 我 R12 寫的驗證器要求「恰一個出口」，但 GDD §164 明定每張探索地圖
**1～3 個出口**（可跨樓層）。新驗證器反而擋掉合法內容。改為 1～3。

**#5（P2）房間自身的拓樸與 ID 唯一性未驗** —— 把同一個房間的格放在地圖兩端，驗證器照樣回傳無錯誤；那等於
在兩塊不相連的空間之間開了一條零成本傳送（§94 房間＝一個移動節點）。房間 ID 重複也沒驗，而連線是用 id
指名的——後面同名房間會悄悄吃掉前面那些連線。兩者都補上，另加重複格檢查。

### 這輪的教訓

1. **「補上發送點」不是修法，收斂成單一來源才是。** #1 連補三輪都沒補完；改成差異比對之後，漏發在結構上
   就不可能發生。#2 同理：同一個「鎖是否生效」判斷散在三處。
2. **抽共用時要驗原本是不是對的。** #3 我把「行為與原實作一致」當成驗證標準，但原實作本身就是壞的。
   抽取重構的驗收條件是「行為正確」，不是「行為不變」。
3. **新增驗證器會同時產生兩種風險。** #4 過嚴（擋掉合法內容）與 #5 過鬆（放行非法內容）都出現了。寫規則
   驗證時，每條規則都要同時準備一個「應通過」與一個「應拒絕」的樣本。

四項地圖修正以回報情境實測：兩出口通過、四出口拒絕、不相連房間拒絕、重複 ID 拒絕、上樓梯畫出 ↑。
兩條引擎修正各自 mutation check。`tsc` 乾淨、verify 全過。

**併行工作現況**（唯讀確認，未修改）：另一工作的四國資料已擴充為各 9 張圖／18 層，且**通過**本輪加嚴後的
驗證器，樓梯 ↑↓ 成對（雲華 10/10，其餘三國 9/9）。#3 修好後那些 ↑ 會正確顯示。HTML 尚未由該工作重新產生。
