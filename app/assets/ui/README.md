...
設施操作由 `app/FacilityChoices.tsx` 使用既有圖示及木框排列：分類與搜尋、左側四筆分頁、右側詳情、底部確認。選取與提交分離，原始命令按鈕提供正式 onClick／disabled，清單可檢視不能執行的項目；沒有候選時顯示空狀態。`app/facilities.css` 定義固定返回鍵、視窗安全邊距及各閱讀區，禁止用浮動圖示擠壓文字。
設施接待畫面使用[四文化設施美術](../facilities/README.md)，取代公會專用底板與共用文字返回鍵。
