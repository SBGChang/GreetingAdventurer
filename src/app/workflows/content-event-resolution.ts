...
const TRANSLATORS: Readonly<Record<EffectDefinitionKind, EffectTranslator>> = {
  ...
  grantItem: (effect, ctx) => {
    if (effect.effectKind !== 'grantItem') return notWired('grantItem', 'unreachable');
  ...
  applyStatus: (effect, ctx) => {
    if (effect.effectKind !== 'applyStatus') return notWired('applyStatus', 'unreachable');
    if (effect.target !== 'actor') {
  ...
  setWorldFact: (effect, ctx) => {
    if (effect.effectKind !== 'setWorldFact') return notWired('setWorldFact', 'unreachable');
    return {
      ok: true,
      messages: [
...
export function dispatchOptionEffects(
  ...
  for (const effectId of effectIds) {
    ...
    const translated = TRANSLATORS[effect.effectKind](effect, ctx);
